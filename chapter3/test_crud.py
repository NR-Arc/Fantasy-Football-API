"""Testing SQLAlchemy CRUD operations."""
import pytest
...
def test_create_players_by_name(db_session):
    ...
    assert player[0].player_id == 2009
...
def test_get_all_performances(db_session):
   ...
   what is expected - all the performances"""
    performances = crud.get_all_performances(db_session, skip=0, limit=18000,
                                            min_last_changed_date=test_date)
...
def test_get_new_performances(db_session):
    """Tests that the count of performances in the database is
   what is expected"""
    performances = crud.get_new_performances(db_session, skip=0, limit=18000,
                                            min_last_changed_date=test_date)
    assert len(performances) == 8224
